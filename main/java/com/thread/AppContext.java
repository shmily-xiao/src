...
public class AppContext {
    ...
    public static Semaphore[] forks;  // 叉子的信息量
    ...
    static{
        ...
        counter = new Semaphore(NUM_OF_PHILO - 1); // 如果有N个哲学家，最多只允许N-1个人同时拥有叉子
    }
    ...
    public static void putOnFork(int index, boolean leftFirst) throws InterruptedException{
        if (leftFirst){
            forks[index].acquire();
            forks[(index + 1) % NUM_OF_PHILO].acquire();// 这个方法会阻塞，直到得到筷子的信息量，或者是抛出异常
        } else {
...
class Philosopher implements Runnable{
    ...
    public void run() {
        while (true){
            try{
                AppContext.counter.acquire(); // 获得哲学家的信息量（N-1）个
                boolean leftFirst = index % 2 == 0;
                AppContext.putOnFork(index, leftFirst);
                System.out.println(name + "正在吃意大利面....");
...
class test{
    public static void main(String[] args) {
        String[] names = {"aa","bb","cc","dd","ee"}; //五位哲学家
