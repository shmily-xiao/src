...
<body>
		<form id ="emailpersonalsend" method="post">
			<div class="right" style="float:left">
				<div>
				
				 <%
					ArrayList<EmailModel> emailmodel = (ArrayList<EmailModel>)request.getAttribute("emailmodeldetail"); 
				
				
				%>
					<input type="button" value="收件人：">
					<input class="f1" type="text" name="acceptperson" onfocusout="if(isSearch(this.value))this.focus()" onkeydown="if(event.keyCode==32) return false"  id = "acceptperson"tabindex="1" placeholder="用户id" value="">
				...
				<div  class="acceptbox" style="float:none">	
					<div style="float:left">
						...
						<input class="f2" type="text" name="emailtheme"  onfocusout="if(isSearch(this.value))this.focus()" onkeydown="if(event.keyCode==32) return false"  id="emailtheme" tabindex="1" placeholder="填写主题" value="<%=emailmodel.get(0).getE_mail_theme()%>">
					</div>
					<div  class="c1"><a href="emailpersonalmodel"><input type="button" style="cursor:pointer"value="返回"/></a></div>
				</div>
				
				<div class="send" >
						
						<textarea id="sendtext" name="sendtext" rows="16" style="resize:none"><%=emailmodel.get(0).getE_mail_detail() %></textarea>
