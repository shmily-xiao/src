...
function register_ajax(){
    ...
    if(!isUsername(username) || username.length <2){
        $("#register_error").addClass("error_color").html("用户名必须是大于两位的中文，请填写你真实的名字"
    ...
    if (!isStudentID(student_id) || student_id.length >16 || student_id.length <8){
        $("#register_error").addClass("error_color").html("请如实填写你的学号，八位以上"
            ).fadeIn(1000).fadeOut(4000);
        $("#studentid").addClass("has-error");
        return false
    }$("#studentid").removeClass("has-error");
    if (student_degree == ''){
        $("#register_error").addClass("error_color").html("请填入生源地"
            ).fadeIn(1000).fadeOut(4000);
    ...
    if (!isIdCard(student_idcard)){
        $("#register_error").addClass("error_color").html("请填入正确的身份证15/18"
            ).fadeIn(1000).fadeOut(4000);
    ...
    if (!isStudentQQ(student_tel) || student_tel.length != 11){
        $("#register_error").addClass("error_color").html("政治面貌不能为空"
            ).fadeIn(1000).fadeOut(4000);
...
function add_option(){
    ...
    var grade = ["应用物理","数学与应用数学","信息与计算科学","光电信息科学与工程"];
    for (num in grade){
...
function evaluation_ajax(){ //综合评价模块
    get_content = {};
    get_content.user_pk = $("#name_pk").attr("alt");
...
function teacher_valuation_ajax(){ //教师评价模块
    get_content = {};
    get_content.pk = $("#name_pk").attr("alt");   //学生学号
    get_content.course = $("#course_name").val();  //课程名字
    get_content.comments_time = $("#course_time").val(); //课程时间
    get_content.teacher_comments = $("#teacher_course_valuation").val(); //课程老师的评价
...
function counselor_valuation_ajax(){ //辅导员评价模块
    get_content = {};
    get_content.pk = $("#name_pk").attr("alt");   //学生学号
    get_content.counselor_comments = $("#counselor_course_valuation").val(); //辅导员的评价
